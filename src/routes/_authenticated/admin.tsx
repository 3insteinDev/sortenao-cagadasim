...
function ActionsTab() {
  ...
  return (
    <div className="space-y-6 max-w-md">
      ...
      <button onClick={async () => { setBusy(true); try { const r = await recalc({}); const profiles = (r as any)?.profiles ?? 0; toast.success(`Recalculado para ${profiles} usuários`); window.location.reload(); } catch (e: any) { toast.error(e?.message ?? "Erro ao recalcular"); } finally { setBusy(false); } }}
